...
public class GraficaFuncion {
	...
	private boolean isSeparatePoints; // Determina si la grafica es punteada o
										// no.
	 ...
	 *            los puntos
	 * @param separate
	 *            indica si se grafican solo los puntos o como l&iacute;nea.
	 * @param x
	 *            intervalo x
	 * @param y
	 *            intervalo y
	 */
	public GraficaFuncion(BigPoint[] bp, boolean separate, BigInterval x,
			BigInterval y, BigInterval intX) {
	...
	public Shape getGraficaFuncion(Point coord, Dimension dim, int xAxis,
		...
		for (BigPoint currBP : puntos) {
			...
			boolean pointInsideOfIntegral = (currBP.x().compareTo(intX.min()) >= 0)
					&& (currBP.x().compareTo(intX.max()) <= 0);
