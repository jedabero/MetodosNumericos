...
public class EcDiffUI extends JPanel implements ChangeListener, ActionListener,
	...
	private static final String strMetodos[] = new String[] { "Euler Simple",
			"Euler Simple Modificado", "Serie de Taylor de orden 2",
			"Runge-Kutta" };
	...
	private void initComponents() {
		...
		eppQx = new EditaPolinomioPanel("Edicion de Q(x)");
		eppPx = new EditaPolinomioPanel("Edicion de P(x)");

		btnCreaEc = new JButton("Crea la ecuacion");
		btnCreaEc.addActionListener(this);
		lblEc = new JLabel();

		pnlMetodos = new JPanel(new GridBagLayout());
		pnlMetodos.setBorder(javax.swing.BorderFactory
				.createTitledBorder("Metodos"));
	...
	private void addComponents() {
		...
		Add.componente(
				...
				new JLabel(
						"<html><center>"
								+ "Resolucion de ecuaciones ordinarias de primer orden"
								...
								+ "</center></html>", JLabel.CENTER), 0, 0, 4,
				1, 1.0, 1.0, GridBagConstraints.NONE, "");
		...
		Add.componente(this, lblEc, 0, 1, 3, 1, 1.0, 1.0,
				GridBagConstraints.BOTH, "Ecuacion diferencial");
		Add.componente(this, btnCreaEc, 3, 1, 1, 1, 1.0, 1.0,
				GridBagConstraints.BOTH, "");

		// 2 - Separator
		Add.componente(this, new JSeparator(JSeparator.HORIZONTAL), 0, 2, 4, 1,
				1.0, 1.0, GridBagConstraints.HORIZONTAL, "");
		...
		// 3 - 1 - 0 - X0 y Xn
		Add.componente(pnlParam, new JLabel("<html>x<sub>0</sub></html>",
				JLabel.CENTER), 0, 0, 1, 1, 1.0, 1.0, GridBagConstraints.NONE,
				"");
		Add.componente(pnlParam, txtX0, 1, 0, 1, 1, 1.0, 1.0,
				GridBagConstraints.BOTH, "");
		Add.componente(pnlParam, new JLabel("<html>x<sub>n</sub></html>",
				JLabel.CENTER), 2, 0, 1, 1, 1.0, 1.0, GridBagConstraints.NONE,
				"");
		Add.componente(pnlParam, txtXn, 3, 0, 1, 1, 1, 1,
				GridBagConstraints.BOTH, "");
		// 3 - 1 - 1 - Y0 y subdivs
		Add.componente(pnlParam, new JLabel("<html>y<sub>0</sub></html>",
				JLabel.CENTER), 0, 1, 1, 1, 1, 1, GridBagConstraints.NONE, "");
		Add.componente(pnlParam, txtY0, 1, 1, 1, 1, 1.0, 1.0,
				GridBagConstraints.BOTH, "");
		Add.componente(pnlParam, new JLabel("Subdivisiones", JLabel.CENTER), 2,
				1, 1, 1, 1, 1, GridBagConstraints.NONE, "");
		Add.componente(pnlParam, spnrN, 3, 1, 1, 1, 1.0, 1.0,
				GridBagConstraints.BOTH, "");
		// 3 - 1 - Panel de par�metros
		Add.componente(pnlMetodos, pnlParam, 0, 1, 4, 2, 1.0, 1.0,
				GridBagConstraints.BOTH, "");
		// 3 - 3 - etiqueta resultado
		Add.componente(pnlMetodos, lblRes, 0, 3, 4, 1, 1.0, 1.0,
				GridBagConstraints.BOTH, "");
		// 3 - panel m�todos
		Add.componente(this, pnlMetodos, 0, 3, 4, 1, 1.0, 1.0,
				GridBagConstraints.BOTH, "");
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (btn.equals(btnCreaEc)) {
			...
			Object o[] = getParams();
			res = edo1.metodoEulerSimple((BigInterval) o[0], (BigDecimal) o[1],
					Integer.parseInt(o[2].toString()));
		} else if (btn.equals(btnFind[1])) {
			Object o[] = getParams();
			res = edo1.metodoEulerSimpleModificado((BigInterval) o[0],
					(BigDecimal) o[1], Integer.parseInt(o[2].toString()));
		} else if (btn.equals(btnFind[2])) {
			Object o[] = getParams();
			res = edo1.metodoSeriesTaylorOrden2((BigInterval) o[0],
					(BigDecimal) o[1], Integer.parseInt(o[2].toString()));
		} else if (btn.equals(btnFind[3])) {
			Object o[] = getParams();
			res = edo1.metodoRungeKutta((BigInterval) o[0], (BigDecimal) o[1],
					Integer.parseInt(o[2].toString()));
		}
		...
		try {
			...
			lblRes.setText("y(" + txtXn.getText() + ") = "
					+ res.stripTrailingZeros().toString());
		} catch (Exception e2) {
	...
	private Object[] getParams() {
		...
		obj[0] = new BigInterval(new BigDecimal(txtX0.getText()),
				new BigDecimal(txtXn.getText()));
