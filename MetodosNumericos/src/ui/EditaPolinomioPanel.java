...
public class EditaPolinomioPanel extends JPanel implements EditaFuncion {
	...
	public void layoutCoefsPanel() {
		...
		for (int i = 0; i < listCoefs.size(); i++) {
			...
			Add.componente(pnlCoefs, listCoefs.get(i), x, y, 1, 1, 1.0, 1.0,
	...
	public Funcion getFuncion() {
		...
		for (int i = 0; i <= grado && noErrors; i++) {
			...
			try {
				...
				JOptionPane
						.showMessageDialog(pnlCoefs,
								"<html>Coeficiente A<sub>" + i + "</sub>: "
										+ (text.isEmpty() ? "vacio" : text)
										+ "</html>", "Error",
	...
	public void stateChanged(ChangeEvent e) {
		...
		if (grado < g) {
			CoeficientePanel tempcp = new CoeficientePanel("<html>A<sub>" + g
					+ "</sub>= </html>");
			listCoefs.add(tempcp);
		} else if (grado > g) {
