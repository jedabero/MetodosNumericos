...
/**
 * La clase {@code Termino} define lo que es un t&eacute;rmino en una
 * funci&oacute;n. Cada t&eacute;rmino tiene los siguientes componentes:
 * <dl>
 * <dt>Coeficiente de t&eacute;rmino {@code A}
 * <dd>La constante que multiplica la funci&oacute;n espec&iacute;fica del
 * t&eacute;rmino.
 * <dt>Una funci&oacute;n {@code F(x)}
 * <dd>Depender&aacute; del tipo de funci&oacute;n; sea un {@code monomio} de
 * grado {@code n}, una funci&oacute;n {@code trigonom&eacute;trica},
 * {@code logar&iacute;tmica}, {@code exponencial}, etc.
 ...
 * </dl>
 * <p>
 * Es usada en la clase {@link Funcion} para definir cada un de los
 * t&eacute;rminos.
 * <p>
 * Creada como redise&ntilde;o de la clase {@link FuncionBase} antes usada para
 * definir funciones.
 * 
...
public class Termino {
	 ...
	 * 
	 * @param a
	 *            el nuevo valor del coeficiente.
	 */
	 ...
	 * 
	 * @param tipoFuncion
	 *            el nuevo tipo de funci&oacute;n.
	 */
	...
	public BigDecimal valorImagen(BigDecimal x) {
		...
		case POLINOMICA:
			return getA().multiply(
					getFuncInterna().valorImagen(x).pow(getGrado()));
		case TRIGONOMETRICA:
			return getA().multiply(
					M.calculaTrig(getFunTrig(), getFuncInterna().valorImagen(x)
							.multiply(getB()), isXinRadians(), isXinDegrees()));
		case EXPONENCIAL:
			return getA().multiply(
					Big.exp(getFuncInterna().valorImagen(x).multiply(getB())));
		case LOGARITMICA:
			return getA().multiply(
					Big.ln(getFuncInterna().valorImagen(x).multiply(getB())));
		case RACIONAL:
			break;
		default:
			return null;
	...
	private void initGenericString() {
		...
		String fint = (hasIntern) ? (isInternMono ? "x" : "(" + fi.getGeneric()
				+ ")") : "null";
		...
		case TRIGONOMETRICA:
			...
			break;
		case EXPONENCIAL:
			gS += "Ae<sup>B" + fint + "</sup>";
			break;
		case LOGARITMICA:
			gS += "Aln(B" + fint + ")";
			break;
		case RACIONAL: /* TODO RACIONAL */
			break;
		default:
			break;
		}
	...
	private void initSpecificString() {
		...
		Funcion fi = getFuncInterna();
		String fint = (hasIntern) ? (isInternMono ? "x" : "("
				+ fi.getSpecific() + ")") : "null";
	...
	/**
	 * Inicializa la representaci&oacute;n espec&iacute;fica y general del
	 * t&eacute;rmino.
	 */
	...
	public String toString() {
		...
		Funcion fi = getFuncInterna();
		String fint = (hasIntern) ? (isInternMono ? "x" : "(" + fi + ")")
				: "null";
		...
		case POLINOMICA:
			...
			break;
		case TRIGONOMETRICA:
			tS = a + "*" + getFunTrig().toString().toLowerCase() + "(" + b
					+ "*" + fint + ")";
			break;
		case EXPONENCIAL:
			tS = a + "*e^(" + b + "*" + fint + ")";
	 ...
	 * 
	 * @param a
	 *            el coeficiente del t&eacute;rmino
	 * @param b
	 *            el coeficiente de la variable
	 * @param f
	 *            el tipo de funci&oacute;n
	 * @param g
	 *            el grado del t&eacute;rmino
	 * @param ft
	 *            el tipo de funci&oacute;n trigonom&eacute;trica
	 * @param interna
	 *            funci&oacute;n interna (f(g(x))
	 * @param mult
	 *            t&eacute;rmino que multiplica este t&eacute;rmino
	 * @throws CustomException
	 */
	public Termino(BigDecimal a, BigDecimal b, Tipo f, int g, FuncionTrig ft,
			Funcion interna, Termino mult) throws CustomException {
		A = a;
		B = b;
		tipoFuncion = f;
		grado = g;
		...
		isInternMono = (hasIntern) ? (funcInterna.toString().compareTo("x") == 0)
				: false;
	...
	/**
	 * Crea un termino constante cuya imagen siempre ser&aacute; el valor de
	 * coef.
	 * 
	 * @param coef
	 ...
	 *            el coeficiente del t&eacute;rmino
	 * @param coefB
	 *            el coeficiente que acompa&ntilde;a a x
	 * @param interna
	...
	/**
	 * Crea un t&eacute;rmino de tipo logar&iacute;tmico con los
	 ...
	 * @throws CustomException
	 *             si el termino t es diferente a este termino.
	 */
			...
			case POLINOMICA:
				if (getGrado() == t.getGrado()) {
					...
					throw new CustomException(
							"Termino a sumar de grado diferente.");
				}
				break;
			case TRIGONOMETRICA:
				if (getFunTrig().equals(t.getFunTrig())) {
					...
					throw new CustomException(
							"Termino trigonometrico a sumar de tipo diferente.");
				}
				break;
			case EXPONENCIAL:
			case LOGARITMICA:
	...
	/*
	 * public Termino multiplicar(BigDecimal multiplicando){ BigDecimal newA =
	 * getA().multiply(multiplicando); Termino multiplicado; return null; }
	 */
	 ...
	 *            Constante B
	 * @param tf
	 *            Tipo de la funci&oacute;n
	 * @param g
	 *            grado del polinomio, si es del tipo
	 * @param ft
	 *            tipos de la funci&oacute;n trigonom&eacute;trica, si es del
	 *            tipo
	 */
	 ...
	 *            constante B
	 * @param ft
	 *            tipo de la funci&oacute;n trigonom&eacute;trica
	 */
	...
	/**
	 * @param ab
	 * @return un intervalo cuyo m&iacute;nimo corresponde al valor de la
	 *         funci&oacute;n en ab.min y cuyo m&aacute;ximo corresponde al
	 *         valor de la funci&oacute;n en ab.m&aacute;x
	 */
	...
	/**
	 * TODO derivada Trigonom&eacute;tricas, Logar&iacute;tmica, Racional y
	 * otras
	 * 
	...
	/**
	 * TODO integral Trigonom&eacute;tricas, Logar&iacute;tmica, Racional y
	 * otras
	 * 
	...
	public Termino integralIndef() {
		...
		case POLINOMICA:
			int g = getGrado() + 1;
			BigDecimal a = getA().divide(BigDecimal.valueOf(g), 15,
					RoundingMode.UP);
