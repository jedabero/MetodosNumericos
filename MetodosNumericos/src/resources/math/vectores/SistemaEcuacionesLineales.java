...
public class SistemaEcuacionesLineales {
	...
	/**
	 * Constructor sin parametros de matriz. Por obligacian se le pedira al
	 * usuario por consola que ingrese el numero de ecuaciones y el numero de
	 * incognitas. Luego se le pedira que ingrese cada uno de los coeficientes.
	 */
	public SistemaEcuacionesLineales() {
		...
		try {
			...
			Logger.getLogger(SistemaEcuacionesLineales.class.getName()).log(
	...
	/**
	 * Constructor que crea una matriz de nxm. El usuario debe ingresar cada uno
	 * de los coeficientes.
	 * 
	 * @param n
	...
	public SistemaEcuacionesLineales(BigDecimal matriz[][]) {
		...
		for (int i = 0; i < numEq; i++) {
			System.arraycopy(matriz[i], 0, matrizCoef.getMatriz()[i], 0,
					matriz[0].length - 1);
	...
	/**
	 * Metodo de Gauss. Vuelve la matriz de coeficientes en una matriz
	 * triangular superior.
	 */
	...
	/*
	 * para que los metodos anteriores funcionen correctamente es necesario que
	 * la diagonal no tenga ningun cero, los metodos a continuacion son para
	 * resolver tal caso. BUT THEY FAIL :( public double[][]
	 * changeRows(double[][] mt, int pos1, int pos2){ double row[] = new
	 * double[mt[0].length]; for (int j = 0; j < mt[0].length; j++) { row[j] =
	 * mt[pos1][j]; mt[pos1][j] = mt[pos2][j]; mt[pos2][j] = row[j]; }
	 * imprimirMatriz("Fila "+pos1+" cambiada por fila "+pos2); return mt;
	 * 
	 * }
	 * 
	 * private boolean isElemZero(double[][] mTemp, int i, int j){ if
	 * (mTemp[i][j]==0) { return true; }else{ return false; } }
	 * 
	 * private int diagElemZeroPos(double[][] mTemp){ for (int i = 0; i <
	 * mTemp.length; i++) { if(isElemZero(mTemp, i, i)){ return i; } } return
	 * -1; }
	 * 
	 * private double[][] checkForCorrectDiagonal(double[][] mTemp) { int
	 * zeroPos = diagElemZeroPos(mTemp);
	 * 
	 * System.out.println("fila"+zeroPos+" con zero"); if (zeroPos>=0) { int
	 * colPos = -1; for (int i = zeroPos; i < mTemp.length; i++) { if
	 * (mTemp[i][zeroPos]!=0) { colPos = i; break; } } if(colPos>=0){ mTemp =
	 * changeRows(mTemp, zeroPos, colPos); }
	 * imprimirMatriz("Matriz lista: ("+zeroPos+"<->"+colPos+")"); for (int i =
	 * 0; i < mTemp.length; i++) { if (mTemp[i][i]==0) { mTemp =
	 * checkForCorrectDiagonal(mTemp); break; } } }
	 * 
	 * return mTemp; }
	 */
