...
public final class M {
	...
	public static Funcion funcionRandom() {
		...
		for (int i = 0; i < randTerminos; i++) {
			r = BigDecimal.valueOf(3 * Math.random() * Math2.randomSign())
					.setScale(2, 2);
			constA[i] = r;
			r = BigDecimal.valueOf(3 * Math.random() * Math2.randomSign())
					.setScale(2, 2);
	...
	public static BigDecimal calculaTrig(FuncionTrig ft, BigDecimal bd,
		...
		if (rad) {
			...
			switch (ft) {
			case SIN:
				r = Big.sin(Big.toRadians(bd));
				break;
			case COS:
				r = Big.cos(Big.toRadians(bd));
				break;
			case TAN:
				r = Big.tan(Big.toRadians(bd));
				break;
			case SEC:
				r = Big.sec(Big.toRadians(bd));
				break;
			case CSC:
				r = Big.csc(Big.toRadians(bd));
				break;
			case COT:
				r = Big.cot(Big.toRadians(bd));
				break;
			default:
				break;
			}
