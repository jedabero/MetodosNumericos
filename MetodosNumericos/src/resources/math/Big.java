...
public final class Big {
	 ...
	 * BigDecimal
	 */
	public static final BigDecimal DOUBLE_MAX = BigDecimal
	 ...
	 * BigDecimal
	 */
	public static final BigDecimal INT_MAX = BigDecimal
	 ...
	 * 
	 * @param bd
	 *            n&uacute;mero a convertir.
	 * @param isMax
	 *            si es un m&aacute;ximo debe ser true, false para un
	 *            m&iacute;nimo.
	...
	public static BigDecimal compareToInt(BigDecimal bd, boolean isMax) {
		...
		int cR = bd.compareTo(intBD); // Se guarda el resultado de la
										// comparaci&oacute;n.
		switch (cR) {
	...
	public static BigDecimal sec(BigDecimal a) {
		...
		try {
			return BigDecimal.ONE.divide(cos(a), 16, RoundingMode.HALF_UP)
					.stripTrailingZeros();
	...
	public static BigDecimal csc(BigDecimal a) {
		try {
			return BigDecimal.ONE.divide(sin(a), 16, RoundingMode.HALF_UP)
					.stripTrailingZeros();
		} catch (ArithmeticException ae) {
	...
	public static BigDecimal logB(BigDecimal x, BigDecimal base) {
		...
		BigDecimal res = num.divide(den, 10, RoundingMode.HALF_EVEN)
				.stripTrailingZeros();
		return res;
	}

	/**
	 * Regresa el numero de Euler elevado a la potencia del valor del
	 * {@code BigDecimal}
	 * 
	 * @see Math2#exp(double)
	 * @param x
	 *            el exponente a elevar {@code e}.
	...
	/**
	 * Regresa el logaritmo natural (base {@code e}) del valor del
	 * {@code BigDecimal}
	 * 
	 ...
	 * @throws Exception
	 */
	public static BigDecimal sqrt(BigDecimal a, BigDecimal tol)
			throws Exception {
		if (a.signum() == -1) {
			throw new Exception("Numero negativo");
		} else {
			...
			while (!endLoop) {
				BigDecimal s_x0 = a.divide(x0, tol.scale(),
						RoundingMode.HALF_EVEN);
				BigDecimal x1 = x0.add(s_x0).divide(TWO, tol.scale(),
						RoundingMode.HALF_EVEN);
	 ...
	 * @throws Exception
	 */
	public static BigDecimal cbrt(BigDecimal a, BigDecimal tol)
			throws Exception {
		tol = tol.stripTrailingZeros();
		BigDecimal x0 = a.divide(THREE, tol.scale(), RoundingMode.HALF_EVEN);
		boolean endLoop = false;
		while (!endLoop) {
			BigDecimal s_x0_2 = a.divide(x0.pow(2), tol.scale(),
					RoundingMode.HALF_EVEN);
			BigDecimal x1 = TWO.multiply(x0).add(s_x0_2)
					.divide(THREE, tol.scale(), RoundingMode.HALF_EVEN);
			BigDecimal err = x0.subtract(x1).abs();
			if (err.compareTo(tol) < 1) {
				endLoop = true;
			}
	 ...
	 * @throws Exception
	 */
	public static BigDecimal nRoot(int n, BigDecimal a, BigDecimal tol)
			throws Exception {
		if ((n % 2 == 0) && (a.signum() == -1)) {
			...
			boolean endLoop = false;
			while (!endLoop) {
				BigDecimal div = a.divide(x0.pow(n - 1), tol.scale(),
						RoundingMode.HALF_EVEN);
				BigDecimal x1 = bdnm1.multiply(x0).add(div)
						.divide(bdn, tol.scale(), RoundingMode.HALF_EVEN);
				BigDecimal err = x0.subtract(x1).abs();
				if (err.compareTo(tol) < 1) {
					endLoop = true;
				}
	 ...
	 * @throws Exception
	 *             si n est&aacute; fuera del rango 0 < n < 1676
	 */
	 ...
	 * @param v
	 * @param k
	 * @return una lista con las conbinaciones en k de los elementos del vector
	 *         v
	 * @throws Exception
	 */
	public static BigDecimal[] listaCombinaciones(BigDecimal v[], int k)
			throws Exception {
		...
		if (poss[posACambiar] < numElem) {
			...
			if (prevPosACambiar >= 0) {
				...
				if (poss[prevPosACambiar] < (numElem - 1)) {
					...
					boolean exit = false;
					if (prevPosACambiar != 0) {
						while ((poss[prevPosACambiar] >= numElem - 1)
								|| (exit && (prevPosACambiar > 0))) {
							prevPosACambiar--;
							poss[prevPosACambiar]++;
	 ...
	 * @throws Exception
	 */
	public static BigDecimal sumaCombinaciones(BigDecimal v[], int k)
			throws Exception {
